...
class Enemy(pygame.sprite.Sprite):  # класс врага
    ...
    def spawn(self, board, type_list):
        enemy_pos = [random.randint(3, 14), random.randint(3, 14)]
        enemy_type = random.choice(type_list)
        enemy_hp = load_enemy(enemy_type)['hp']
        board[enemy_pos[0]][enemy_pos[1]] = f'{enemy_type}, {enemy_hp}'
    #
